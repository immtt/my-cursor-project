...
def _optional_number(values, header_map, header_name, cast, default=0):
    if header_name not in header_map:
        return default
    value = values[header_map[header_name]]
    if value in (None, ""):
        return default
    return cast(value)
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            if dataset_type == "system":
                ...
                est_duration = _optional_number(values, header_map, "预计时效", int)
                obj = SysSuggest(
