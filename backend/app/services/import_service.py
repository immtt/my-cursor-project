...
def _optional_cell_value(values, header_map, column_name, default=0):
    idx = header_map.get(column_name)
    if idx is None:
        return default
    return values[idx] if values[idx] not in (None, "") else default
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            if dataset_type == "system":
                est_distance = float(_optional_cell_value(values, header_map, "预计公里数"))
                est_duration = int(_optional_cell_value(values, header_map, "预计时效"))
                obj = SysSuggest(
