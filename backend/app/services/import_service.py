...
def _required_text(values, header_map, column_name: str) -> str:
    value = values[header_map[column_name]]
    if value is None:
        raise ValueError(f"{column_name}存在空值")
...
def _optional_float(values, header_map, column_name: str) -> float:
...
def _optional_int(values, header_map, column_name: str) -> int:
    idx = header_map.get(column_name)
    if idx is None:
        return 0
    value = values[idx]
