...
def _optional_cell(values, header_map, column_name, default):
...
def import_excel(db: Session, dataset_type: str, file_path: str):
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            waybill_no = _text_cell(values[header_map["运单号"]])
            route_line = _text_cell(values[header_map["归属线路"]])
            warehouse_name = _text_cell(values[header_map["始发仓库"]])
            stores = _text_cell(values[header_map["拼载门店"]])
            ...
            if dataset_type == "system":
                est_distance = float(_optional_cell(values, header_map, "预计公里数", 0))
                est_duration = int(_optional_cell(values, header_map, "预计时效", 0))
                obj = SysSuggest(
