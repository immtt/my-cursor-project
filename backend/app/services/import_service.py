...
def _optional_cell_value(values: list, header_map: dict[str, int], column_name: str, default):
    column_idx = header_map.get(column_name)
    if column_idx is None:
        return default
    return values[column_idx] or default
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            if dataset_type == "system":
                est_distance = float(_optional_cell_value(values, header_map, "预计公里数", 0))
                est_duration = int(_optional_cell_value(values, header_map, "预计时效", 0))
                obj = SysSuggest(
