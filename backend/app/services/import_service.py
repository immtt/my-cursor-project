...
def _optional_numeric(values: list, header_map: dict[str, int], header: str, default=0):
    idx = header_map.get(header)
    if idx is None:
        return default
    return values[idx] or default
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            if dataset_type == "system":
                est_distance = float(_optional_numeric(values, header_map, "预计公里数"))
                est_duration = int(_optional_numeric(values, header_map, "预计时效"))
                obj = SysSuggest(
