...
def _optional_value(values, header_map, column):
    ...
    for row_no in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            if dataset_type == "system":
                est_distance = float(_optional_value(values, header_map, "预计公里数") or 0)
                est_duration = int(_optional_value(values, header_map, "预计时效") or 0)
                obj = SysSuggest(
