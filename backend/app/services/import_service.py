...
def _optional_float(values, header_map: dict[str, int], header: str, default: float = 0.0) -> float:
...
def _optional_int(values, header_map: dict[str, int], header: str, default: int = 0) -> int:
    idx = header_map.get(header)
    if idx is None:
        return default
    return int(values[idx] or default)
