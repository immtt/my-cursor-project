...
def test_system_import_missing_optional_estimates_defaults_to_zero(db_session, tmp_path):
    file_path = tmp_path / "system_required_only.xlsx"
    wb = Workbook()
    sheet = wb.active
    sheet.append(["排线日期", "运单号", "归属线路", "始发仓库", "拼载门店", "配送体积", "装载率"])
    sheet.append(["2026-04-20", "SYS001", "线路A", "仓库1", "门店甲,门店乙", 10, 77])
    wb.save(file_path)
